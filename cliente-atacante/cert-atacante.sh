...
openssl req -new -x509 -key certs/atacante.key -out certs/atacante.crt \
	-days 365 -subj "/C=AR/ST=Buenos Aires/L=CABA/O=Hacker/CN=AtacanteMalisioso"
echo ""
echo "Certificado autofirmado creado:"
openssl x509 -in certs/atacante.crt -text -noout | grep -E "Subject:|Issuer:|Not Before|Not After|Public-Key"